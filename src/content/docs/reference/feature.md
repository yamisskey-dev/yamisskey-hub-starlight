...
### DAO参加者
- 運営しているサービス内の情報を閲覧できない

### 管理人
- IPアドレス保存設定をしない
- ユーザーのIPアドレスを確認できる（ユーザーのIPアドレスを保存していない）
- 参加していないチャットルームの内容を読める（通報対応する場合のみ）
- ユーザーのアクティブ状態を閲覧できる（独自機能）
- ユーザーのアカウントを削除できる
- モデログ（モデレーターの活動記録）を確認できる
...
- ユーザーの鍵ノートとDMを読めない
- ユーザーが添付したファイルを閲覧できる
- ユーザーのモデレーションノート（モデレーター間でだけ共有されるメモ）を閲覧編集できる
- 管理人が許可したマニュアルロールをユーザーに付与できる
- ユーザーを凍結できる
- 非公開のフォロイーとフォロワーが誰か閲覧できない（独自機能）
- 他サーバーとの連合状況を閲覧できる
- ユーザーの登録理由を閲覧できる（独自機能）
- 承認制で新規登録希望者を承認/拒否できる（独自機能）
- ユーザーのパスワードをリセットできる
- ユーザーのアイコンを解除できる
- ユーザーのバナーを解除できる
- モデログ（モデレーターの活動記録）を確認できない
