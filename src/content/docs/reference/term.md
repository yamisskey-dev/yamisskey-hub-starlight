...
やみすきーは「承認弱者」が自分らしくいられる場所です。「病み」と「闇」を抱える者たちが互いを理解し支え合える場所を目指しています。

本規約および[モデレーション原則](/reference/moderation/)は、以下の4つの倫理的原則に基づきます：

1. **自律尊重原則**: すべてのユーザーは、自己の意思に基づいて行動する権利を有する。他者の自律を侵害してはならない
2. **危害原則**: ユーザーの行動は、他者に具体的な危害を与える場合に限り制限される。不快、不同意、価値観の相違は危害に該当しない
3. **明示性原則**: 禁止される行為は明示的に列挙される。列挙されていない行為は禁止されない。類推解釈および拡大解釈は禁止
4. **平等適用原則**: 本規約は、運営者を含むすべてのユーザーに平等に適用される
...
### 1項（運営組織）
本サービスは、YAMI DAOが運営します。運営体制は以下の通りです：

1. **YAMI DAO**: 本サービスの最高意思決定機関。規約改定、重大な運営方針の決定を行う
2. **管理人**: DAOから委任を受け、日常的な運営を担当する
3. **モデレーター**: 管理人が任命し、モデレーション業務を担当する
4. **YAMII**: AIによる自動モデレーション支援システム

### 2項（YAMIIの役割）
YAMIIは、以下の役割を担います：

1. **判定業務**: 通報された投稿について、本規約および[モデレーション原則](/reference/moderation/)に基づき、違反の有無を判定する
...
### 3項（モデレーション権限）
モデレーターは、本規約に基づき、本サービスのモデレーション（通報された投稿の確認、削除、ユーザーへの警告、アカウントの凍結等の措置）を行います。モデレーションは原則として通報ベースで行い、予防的な監視は行いません。ただし、技術的な異常検知（大量投稿、システム攻撃等）については、サーバーの安定運営のために自動的に検出することがあります。

### 4項（運営の制限）
運営（管理人、モデレーター）は、以下の制限に従います：

1. **明示性原則の遵守**: 本規約およびモデレーション原則に明記されていない行為を理由とした処分を行ってはならない
2. **平等適用**: 本規約は運営者自身にも適用される。運営者が規約に違反した場合、DAOによる処分の対象となる
3. **理由の開示**: すべての措置について、違反した条項を明示して理由を開示する
4. **合議制**: 一時停止以上の処分については、複数のモデレーターによる合議を経て決定する
5. **透明性の確保**: 定期的に運営の活動状況を公開する

### 5項（遵守義務と異議申し立て）
ユーザーは、本規約の指示に従って行動する義務を負います。処分に不服がある場合は、DAOに対して異議を申し立てることができます。異議申し立ては敵対行為とみなされません。

### 6項（紛争への介入原則）
運営は、ユーザー間の紛争について、当事者間の自主的な解決を原則とし、原則として介入しません。ただし、紛争が本規約に違反する場合は、この限りではありません。
...
### 1項（変更の手続き）
本規約の変更は、以下の手続きに従って行われます：

1. **提案**: 変更案はYAMI DAOに提出される
2. **公開期間**: 変更案は最低14日間公開され、ユーザーからの意見を受け付ける
3. **投票**: YAMI DAOにおいて投票を実施する
4. **可決要件**: 投票参加者の過半数の賛成により可決される
5. **施行**: 可決された変更は、施行日を明示した上で発効する

### 2項（緊急変更）
以下の場合に限り、前項の手続きを経ずに緊急の変更を行うことができます：

1. 法令の改正により即時対応が必要な場合
2. セキュリティ上の重大な脆弱性が発見された場合
3. サーバーの運営に重大な支障が生じている場合

緊急変更を行った場合は、変更後速やかにDAOに報告し、事後承認を得るものとします。

### 3項（変更通知義務）
本規約の変更にあたっては、変更内容、変更理由、および効力発生時期を事前に通知します。

### 4項（禁止事項の追加制限）
新たな禁止事項を追加する変更については、以下の追加要件を満たす必要があります：

1. 追加の必要性を具体的に説明すること
2. 4つの倫理的原則（自律尊重、危害原則、明示性、平等適用）との整合性を示すこと
3. 投票参加者の3分の2以上の賛成を得ること

### 5項（倫理的原則の改定制限）
4つの倫理的原則（自律尊重原則、危害原則、明示性原則、平等適用原則）の改定または廃止については、**YAMI DAO構成員全員の賛成（全会一致）** を必要とします。この要件は、本規約の最上位規範としての4原則の安定性を確保するためのものです。
...
- 2025年12月25日　[改定](https://github.com/yamisskey-dev/yamisskey-hub-starlight/commit/bd93b2e)：第14条にライセンス制限・AI学習禁止条項を追加（X対抗条項）
- 2026年1月22日　改定：4つの倫理的原則の明記、運営体制の再構築（YAMII導入）、DAO投票による規約改定手続きの導入